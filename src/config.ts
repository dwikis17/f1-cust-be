...
const env = z.object({
  ...
  BITESHIP_ORIGIN_LATITUDE: z.coerce.number().finite().min(-90).max(90).optional(),
  BITESHIP_ORIGIN_LONGITUDE: z.coerce.number().finite().min(-180).max(180).optional(),
  MAPBOX_PERMANENT_GEOCODING_TOKEN: z.string().trim().optional(),
...
export const config = {
  ...
  biteshipOriginLatitude: env.BITESHIP_ORIGIN_LATITUDE,
  biteshipOriginLongitude: env.BITESHIP_ORIGIN_LONGITUDE,
  mapboxPermanentGeocodingToken: env.MAPBOX_PERMANENT_GEOCODING_TOKEN,
